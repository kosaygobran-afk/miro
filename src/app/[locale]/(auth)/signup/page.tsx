...
export const generateMetadata = async ({ params }: { params: { locale: string } }) => {
  ...
  return {
    title: messages.pages.auth.signup.title,
      ...
      <div className="mx-auto max-w-md px-6 py-12">
        <div className="bg-surface rounded-lg p-6">
          ...
          <form className="space-y-4">
            <div>
              <label className="block mb-2 text-muted-foreground font-medium">{t('form.name')}</label>
              <input type="text" placeholder={t('form.name')} className="w-full px-4 py-2 border border-border-control rounded-lg bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
          ...
          <div className="mt-6 p-4 bg-border-subtle rounded text-sm text-muted-foreground">
            <p>
              ...
              In production, this form would create a new MIRO account.
            </p>
          </div>
